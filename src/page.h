...
#ifndef CROWXLA_PAGE_H
#define CROWXLA_PAGE_H
...
// forked version of the rx_slow_hash from scala
...
// from scala to use me_rx_slow_hash
bool
...
struct tx_details
{
    ...
    get_mstch_map() const
    {
        ...
        const double& xla_amount = XLA_AMOUNT(fee);
          ...
          mainnet_url {_mainnet_url}
{
    ...
    template_file["css_styles"]      = xlaeg::read(TMPL_CSS_STYLES);
    template_file["header"]          = xlaeg::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(xlaeg::read(TMPL_INDEX2));
    template_file["mempool"]         = xlaeg::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(xlaeg::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = xlaeg::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["block"]           = get_full_page(xlaeg::read(TMPL_BLOCK));
    template_file["randomx"]         = get_full_page(xlaeg::read(TMPL_RANDOMX));
    template_file["tx"]              = get_full_page(xlaeg::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(xlaeg::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(xlaeg::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(xlaeg::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(xlaeg::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(xlaeg::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(xlaeg::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(xlaeg::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(xlaeg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(xlaeg::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(xlaeg::read(TMPL_SEARCH_RESULTS));
    template_file["tx_details"]      = xlaeg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_table_header"] = xlaeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = xlaeg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");
}
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , xlaeg::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    //double blk_size_median = xlaeg::calc_median(blk_sizes.begin(), blk_sizes.end());
...
show_block(uint64_t _blk_height)
{
    ...
    string blk_timestamp = xlaeg::timestamp_to_str_gm(blk.timestamp);
    ...
    for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
    {
        ...
        //string mixin_times_scale = xlaeg::timestamps_time_scale(mixin_timestamps,
        //                                                        server_timestamp);
    ...
    context["sum_fees"]
            = xlaeg::xla_amount_to_str(sum_fees, "{:0.6f}", false);

    // get xla in the block reward
    context["blk_reward"]
            = xlaeg::xla_amount_to_str(txd_coinbase.xla_outputs - sum_fees, "{:0.6f}");
...
show_block(string _blk_hash)
{
    ...
    if (!xlaeg::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0, bool refresh_page = false)
{
    ...
    if (!xlaeg::parse_str_secret_key(tx_hash_str, tx_hash))
    {
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = xlaeg::timestamp_to_str_gm(tx_recieve_timestamp);
...
show_ringmembers_hex(string const& tx_hash_str)
{
    ...
    vector<txin_to_key> input_key_imgs = xlaeg::get_key_images(tx);
...
show_ringmemberstx_jsonhex(string const& tx_hash_str)
{
    ...
    // private scala networks, so we can easly put these
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Scala address not provided!");
    }
    ...
    // parse string representing given scala address
    cryptonote::address_parse_info address_info;

    if (!xlaeg::parse_str_address(xla_address_str,  address_info, nettype))
    {
    ...
    if (!xlaeg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!xlaeg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = xlaeg::timestamp_to_str_gm(blk.timestamp);
    ...
    mstch::map context {
            ...
            {"tx_fee"               , xlaeg::xla_amount_to_str(txd.fee, "{:0.12f}", true)},
            {"blk_timestamp"        , blk_timestamp},
    ...
    string server_time_str = xlaeg::timestamp_to_str_gm(server_timestamp, "%F");
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
        ...
        outputs.push_back(mstch::map {
                ...
                {"amount"                , xlaeg::xla_amount_to_str(outp.second)},
    ...
    for (const txin_to_key& in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", xlaeg::xla_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& abs_offset: absolute_offsets)
        {
            ...
            public_key mixin_tx_pub_key
                    = xlaeg::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = xlaeg::get_ouputs_tuple(mixin_tx);
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , xlaeg::xla_amount_to_str(amount)}
                });
...
//
//                        if (!xlaeg::generate_key_image(derivation,
...
//                                << endl;
    ...
    context["possible_spending"] = xlaeg::xla_amount_to_str(
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = xlaeg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , xlaeg::xla_amount_to_str(tx_change.amount)},
                        {"has_payment_id"     , (payment_id  != null_hash)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id"         , pid_str},
                        {"payment_id8"        , pid8_str},
                ...
                for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , xlaeg::xla_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , xlaeg::xla_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   xlaeg::xla_amount_to_str(sum_outputs_amounts)});
                ...
                tx_cd_data["min_mix_time"]     = xlaeg::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = xlaeg::timestamp_to_str_gm(max_mix_timestamp);
    ...
    else
    {
        ...
        if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
        {
            ...
            // used in myscala and openscala projects.
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        xlaeg::xla_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_xla_sum"] = xlaeg::xla_amount_to_str(outputs_xla_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = xlaeg::xla_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            tx_context["inputs_xla_sum"] = xlaeg::xla_amount_to_str(inputs_xla_sum);
            ...
            for (mstch::node& input_node: inputs)
            {
                ...
                amount = xlaeg::xla_amount_to_str(real_amounts.at(input_idx));
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_scala_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!xlaeg::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = xlaeg::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = xlaeg::decrypt(
    ...
    context.insert({"address"        , REMOVE_HASH_BRAKETS(
            xlaeg::print_address(address_info, nettype))});
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = xlaeg::get_tx_pub_key_from_received_outs(tx);
        ...
        mstch::map output_info {
                ...
                {"amount"              , xlaeg::xla_amount_to_str(xla_amount)},
                {"tx_hash"             , REMOVE_HASH_BRAKETS(fmt::format("{:s}", td.m_txid))},
                {"timestamp"           , xlaeg::timestamp_to_str_gm(blk_timestamp)},
    ...
    if (total_xla > 0)
    {
        ...
        context["total_xla"] = xlaeg::xla_amount_to_str(total_xla);
    }
...
search(string search_text)
{
    ...
    // check if scala address is given based on its length
    ...
    if (search_str_length == 95)
    {
        ...
        if (!xlaeg::parse_str_address(search_text, address_info, nettype_addr))
        {
    ...
    // check if integrated scala address is given based on its length
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    string address_str      = xlaeg::print_address(address_info, nettype);
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", xlaeg::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = xlaeg::timestamp_to_str_gm(tx_timestamp);
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!xlaeg::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , xlaeg::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Scala address not provided";
        return j_response;
    }
    ...
    if (!xlaeg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse scala address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_scala_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get scala network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"scala_version_full" , string {SCALA_VERSION_FULL}},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = XLA_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
            {"tx_fee"                , xlaeg::xla_amount_to_str(txd.fee, "{:0.12f}", false)},
            {"tx_fee_micro"          , xlaeg::xla_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , xlaeg::xla_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  xlaeg::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = xlaeg::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = xlaeg::timestamp_to_str_gm(max_mix_timestamp);
    ...
    context["inputs_xla_sum"]           = xlaeg::xla_amount_to_str(inputs_xla_sum);
    ...
    context["outputs_xla_sum"] = xlaeg::xla_amount_to_str(outputs_xla_sum);
...
)
{
    ...
    {
        // get mixins in time scale for visual representation
        pair<string, double> mixin_times_scale = xlaeg::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    // https://github.com/scala-project/scala/pull/1358/commits/7abfc5474c0f86e16c405f154570310468b635c2
    txd.pk = xlaeg::get_tx_pub_key_from_received_outs(tx);
...
bool
get_scala_network_info(json& j_info)
{
...
get_footer()
{
    ...
    string footer_html = mstch::render(xlaeg::read(TMPL_FOOTER), footer_context);
